...
function uidExist($conn,$username){
 ...
 $sql= "SELECT * FROM admin WHERE username=?"; //scopes to check db for userid and userEmail
...
function emailExist($conn,$email){
  //? is a placeholder, due to prepared statements,statements are sent to db 1st then placeholder are filled user provides data 
 $sql= "SELECT * FROM admin WHERE email=?"; //scopes to check db for userid and userEmail
 // prepared statement to submit data in a proper way
 $stmt=$conn->prepare($sql); // initialising new prepared statement
...
if(emailExist($conn,$email,) !== false){
  ...
  exit();
    ...
    $query = "INSERT into admin(fullname,username,email,contact,address,password) VALUES('" . $fullname . "','" . $username . "','" . $email . "','" . $contact . "','" . $address ."','" . $pwd ."')";
